...
def ATR(df, n):
    ...
    Example::
        ...
        from ta_lib import TqApi, TqAuth, TqSim
        from ta_lib.ta import ATR
    ...
    new_df["atr"] = ta_lib.tafunc.ma(new_df["tr"], n)
    return new_df
...
def BIAS(df, n):
    ...
    """
    ma1 = ta_lib.tafunc.ma(df["close"], n)
...
def BOLL(df, n, p):
    ...
    """
    new_df = pd.DataFrame()
    mid = ta_lib.tafunc.ma(df["close"], n)
...
def DMI(df, n, m):
    ...
    hd = df["high"] - pre_high
    ld = pre_low - df["low"]
    admp = ta_lib.tafunc.ma(pd.Series(np.where((hd > 0) & (hd > ld), hd, 0)), n)
    admm = ta_lib.tafunc.ma(pd.Series(np.where((ld > 0) & (ld > hd), ld, 0)), n)
    ...
    new_df["adx"] = ta_lib.tafunc.ma(ad, m)
...
def KDJ(df, n, m1, m2):
    ...
    new_df["k"] = ta_lib.tafunc.sma(rsv, m1, 1)
    new_df["d"] = ta_lib.tafunc.sma(new_df["k"], m2, 1)
...
def MACD(df, short, long, m):
    ...
    """
    new_df = pd.DataFrame()
    eshort = ta_lib.tafunc.ema(df["close"], short)
    elong = ta_lib.tafunc.ema(df["close"], long)
    new_df["diff"] = eshort - elong
    new_df["dea"] = ta_lib.tafunc.ema(new_df["diff"], m)
...
def _sar(open, high, low, close, range_high, range_low, n, step, maximum):
    ...
    sar = np.empty_like(close).astype(float)
    sar[:n] = np.NAN
    af = 0
    ep = 0
...
def RSI(df, n):
    ...
    """
    lc = df["close"].shift(1)
    rsi = ta_lib.tafunc.sma(pd.Series(np.where(df["close"] - lc > 0, df["close"] - lc, 0)), n, 1) / \
          ta_lib.tafunc.sma(np.absolute(df["close"] - lc), n, 1) * 100
...
def DMA(df, short, long, m):
    ...
    """
    new_df = pd.DataFrame()
    new_df["ddd"] = ta_lib.tafunc.ma(df["close"], short) - ta_lib.tafunc.ma(df["close"], long)
    new_df["ama"] = ta_lib.tafunc.ma(new_df["ddd"], m)
    return new_df
...
def EXPMA(df, p1, p2):
    ...
    """
    new_df = pd.DataFrame()
    new_df["ma1"] = ta_lib.tafunc.ema(df["close"], p1)
    new_df["ma2"] = ta_lib.tafunc.ema(df["close"], p2)
    return new_df
...
def CR(df, n, m):
    ...
    new_df["crma"] = ta_lib.tafunc.ma(new_df["cr"], m).shift(int(m / 2.5 + 1))
    return new_df
...
def CDP(df, n):
    ...
    new_df["ah"] = ta_lib.tafunc.ma(cdp + pt, n)
    new_df["al"] = ta_lib.tafunc.ma(cdp - pt, n)
    new_df["nh"] = ta_lib.tafunc.ma(2 * cdp - df["low"], n)
    new_df["nl"] = ta_lib.tafunc.ma(2 * cdp - df["high"], n)
    return new_df
...
def HCL(df, n):
    ...
    """
    new_df = pd.DataFrame()
    new_df["mah"] = ta_lib.tafunc.ma(df["high"], n)
    new_df["mal"] = ta_lib.tafunc.ma(df["low"], n)
    new_df["mac"] = ta_lib.tafunc.ma(df["close"], n)
    return new_df
...
def ENV(df, n, k):
    ...
    """
    new_df = pd.DataFrame()
    new_df["upper"] = ta_lib.tafunc.ma(df["close"], n) * (1 + k / 100)
    new_df["lower"] = ta_lib.tafunc.ma(df["close"], n) * (1 - k / 100)
    return new_df
...
def PUBU(df, m):
    ...
    """
    pb = (ta_lib.tafunc.ema(df["close"], m) + ta_lib.tafunc.ma(df["close"], m * 2) + ta_lib.tafunc.ma(df["close"], m * 4)) / 3
...
def BBI(df, n1, n2, n3, n4):
    ...
    """
    bbi = (ta_lib.tafunc.ma(df["close"], n1) + ta_lib.tafunc.ma(df["close"], n2) + ta_lib.tafunc.ma(df["close"], n3) + ta_lib.tafunc.ma(
        df["close"], n4)) / 4
...
def DKX(df, m):
    ...
    new_df["d"] = ta_lib.tafunc.ma(new_df["b"], m)
    return new_df
...
def BBIBOLL(df, n, m):
    ...
    """
    new_df = pd.DataFrame()
    new_df["bbiboll"] = (ta_lib.tafunc.ma(df["close"], 3) + ta_lib.tafunc.ma(df["close"], 6) + ta_lib.tafunc.ma(df["close"],
                                                                                           12) + ta_lib.tafunc.ma(
        df["close"], 24)) / 4
...
def ADTM(df, n, m):
    ...
    new_df["adtmma"] = ta_lib.tafunc.ma(new_df["adtm"], m)
    return new_df
...
def B3612(df):
    ...
    """
    new_df = pd.DataFrame()
    new_df["b36"] = ta_lib.tafunc.ma(df["close"], 3) - ta_lib.tafunc.ma(df["close"], 6)
    new_df["b612"] = ta_lib.tafunc.ma(df["close"], 6) - ta_lib.tafunc.ma(df["close"], 12)
    return new_df
...
def DBCD(df, n, m, t):
    ...
    """
    new_df = pd.DataFrame()
    bias = (df["close"] - ta_lib.tafunc.ma(df["close"], n)) / ta_lib.tafunc.ma(df["close"], n)
    dif = bias - bias.shift(m)
    new_df["dbcd"] = ta_lib.tafunc.sma(dif, t, 1)
    new_df["mm"] = ta_lib.tafunc.ma(new_df["dbcd"], 5)
    return new_df
...
def DDI(df, n, n1, m, m1):
    ...
    new_df["ddi"] = diz - dif
    new_df["addi"] = ta_lib.tafunc.sma(new_df["ddi"], n1, m)
    new_df["ad"] = ta_lib.tafunc.ma(new_df["addi"], m1)
    return new_df
...
def LWR(df, n, m):
    ...
    new_df = pd.DataFrame(data=list(ta_lib.tafunc.sma(rsv, m, 1)), columns=["lwr"])
    return new_df
...
def MASS(df, n1, n2):
    ...
    """
    ema1 = ta_lib.tafunc.ema(df["high"] - df["low"], n1)
    ema2 = ta_lib.tafunc.ema(ema1, n1)
...
def MI(df, n):
    ...
    new_df["mi"] = ta_lib.tafunc.sma(new_df["a"], n, 1)
    return new_df
...
def MICD(df, n, n1, n2):
    ...
    ami = ta_lib.tafunc.sma(mi, n, 1)
    new_df["dif"] = ta_lib.tafunc.ma(ami.shift(1), n1) - ta_lib.tafunc.ma(ami.shift(1), n2)
    new_df["micd"] = ta_lib.tafunc.sma(new_df["dif"], 10, 1)
    return new_df
...
def MTM(df, n, n1):
    ...
    new_df["mtmma"] = ta_lib.tafunc.ma(new_df["mtm"], n1)
    return new_df
...
def PRICEOSC(df, long, short):
    ...
    Example::
        ...
        from ta_lib import TqApi, TqAuth, TqSim
        from ta_lib.ta import PRICEOSC
    ...
    """
    ma_s = ta_lib.tafunc.ma(df["close"], short)
    ma_l = ta_lib.tafunc.ma(df["close"], long)
...
def PSY(df, n, m):
    ...
    Example::
        ...
        from ta_lib import TqApi, TqSim
        from ta_lib.ta import PSY
    ...
    """
    new_df = pd.DataFrame()
    new_df["psy"] = ta_lib.tafunc.count(df["close"] > df["close"].shift(1), n) / n * 100
    new_df["psyma"] = ta_lib.tafunc.ma(new_df["psy"], m)
    return new_df
...
def RC(df, n):
    ...
    new_df = pd.DataFrame(data=list(ta_lib.tafunc.sma(rc.shift(1), n, 1)), columns=["arc"])
    return new_df
...
def RCCD(df, n, n1, n2):
    ...
    new_df = pd.DataFrame()
    rc = df["close"] / df["close"].shift(n)
    arc = ta_lib.tafunc.sma(rc.shift(1), n, 1)
    new_df["dif"] = ta_lib.tafunc.ma(arc.shift(1), n1) - ta_lib.tafunc.ma(arc.shift(1), n2)
    new_df["rccd"] = ta_lib.tafunc.sma(new_df["dif"], n, 1)
    return new_df
...
def ROC(df, n, m):
    ...
    Example::
        ...
        from ta_lib import TqApi, TqAuth
        from ta_lib.ta import ROC
    ...
    new_df["rocma"] = ta_lib.tafunc.ma(new_df["roc"], m)
    return new_df
...
def SLOWKD(df, n, m1, m2, m3):
    ...
    fastk = ta_lib.tafunc.sma(rsv, m1, 1)
    new_df["k"] = ta_lib.tafunc.sma(fastk, m2, 1)
    new_df["d"] = ta_lib.tafunc.sma(new_df["k"], m3, 1)
    return new_df
...
def SRDM(df, n):
    ...
    dmf = np.where((df["high"] + df["low"]) >= (df["high"].shift(1) + df["low"].shift(1)), 0,
                   np.where(np.absolute(df["high"] - df["high"].shift(1)) > np.absolute(df["low"] - df["low"].shift(1)),
                            np.absolute(df["high"] - df["high"].shift(1)), np.absolute(df["low"] - df["low"].shift(1))))
    admz = ta_lib.tafunc.ma(pd.Series(dmz), 10)
    admf = ta_lib.tafunc.ma(pd.Series(dmf), 10)
    new_df["srdm"] = np.where(admz > admf, (admz - admf) / admz, np.where(admz == admf, 0, (admz - admf) / admf))
    new_df["asrdm"] = ta_lib.tafunc.sma(new_df["srdm"], n, 1)
    return new_df
...
def ZDZB(df, n1, n2, n3):
    ...
    new_df["b"] = ta_lib.tafunc.ma(a, n2)
    new_df["d"] = ta_lib.tafunc.ma(a, n3)
    return new_df
...
def DPO(df):
    ...
    """
    dpo = df["close"] - (ta_lib.tafunc.ma(df["close"], 20)).shift(11)
...
def LON(df):
    ...
    new_df["ma1"] = ta_lib.tafunc.ma(new_df["lon"], 10)
    return new_df
...
def SHORT(df):
    ...
    new_df["ma1"] = ta_lib.tafunc.ma(new_df["short"], 10)
    return new_df
...
def MV(df, n, m):
    ...
    """
    new_df = pd.DataFrame()
    new_df["mv1"] = ta_lib.tafunc.sma(df["volume"], n, 1)
    new_df["mv2"] = ta_lib.tafunc.sma(df["volume"], m, 1)
    return new_df
    ...
    new_df["a"] = np.absolute(np.where(df["close"] > df["close"].shift(1),
                                       ...
                                       np.where(df["close"] < df["close"].shift(1), df["close"] - np.where(
                                           df["close"].shift(1) > df["high"], df["close"].shift(1), df["high"]),
                                                0)).cumsum())
    new_df["b"] = ta_lib.tafunc.sma(new_df["a"], n, 1)
    new_df["e"] = ta_lib.tafunc.sma(new_df["a"], m, 1)
    return new_df
...
def VOSC(df, short, long):
    ...
    """
    vosc = (ta_lib.tafunc.ma(df["volume"], short) - ta_lib.tafunc.ma(df["volume"], long)) / ta_lib.tafunc.ma(df["volume"], short) * 100
...
def VRSI(df, n):
    ...
    """
    vrsi = ta_lib.tafunc.sma(
        pd.Series(np.where(df["volume"] - df["volume"].shift(1) > 0, df["volume"] - df["volume"].shift(1), 0)), n,
        1) / ta_lib.tafunc.sma(np.absolute(df["volume"] - df["volume"].shift(1)), n, 1) * 100
...
def MA(df, n):
    ...
    """
    new_df = pd.DataFrame(data=list(ta_lib.tafunc.ma(df["close"], n)), columns=["ma"])
    return new_df
...
def SMA(df, n, m):
    ...
    """
    new_df = pd.DataFrame(data=list(ta_lib.tafunc.sma(df["close"], n, m)), columns=["sma"])
    return new_df
...
def EMA(df, n):
    ...
    """
    new_df = pd.DataFrame(data=list(ta_lib.tafunc.ema(df["close"], n)), columns=["ema"])
    return new_df
...
def EMA2(df, n):
    ...
    """
    new_df = pd.DataFrame(data=list(ta_lib.tafunc.ema2(df["close"], n)), columns=["ema2"])
    return new_df
...
def TRMA(df, n):
    ...
    """
    new_df = pd.DataFrame(data=list(ta_lib.tafunc.trma(df["close"], n)), columns=["trma"])
    return new_df
...
def BS_VALUE(df, quote, r=0.025, v=None):
    ...
    Args:
        ...
        quote (ta_lib.objs.Quote): 需要计算理论价的期权对象，其标的合约应该是 df 序列对应的合约，否则返回序列值全为 nan
    ...
    Example1::
        ...
        from ta_lib import TqApi, TqAuth
        from ta_lib.ta import BS_VALUE
    ...
    Example2::
        ...
        from ta_lib import TqApi, TqAuth
        from ta_lib.ta import BS_VALUE
        from ta_lib.tafunc import get_his_volatility
    ...
    if v is None:
        v = ta_lib.tafunc._get_volatility(df["close"], df["duration"], quote.trading_time)
        if math.isnan(v):
            return pd.DataFrame(np.full_like(df["close"], float('nan')), columns=["bs_price"])
    t = ta_lib.tafunc._get_t_series(df["datetime"], df["duration"], quote.expire_datetime)
    return pd.DataFrame(data=list(ta_lib.tafunc.get_bs_price(df["close"], quote.strike_price, r, v, t, quote.option_class)),
                        columns=["bs_price"])
...
def OPTION_GREEKS(df, quote, r=0.025, v=None):
    ...
    Args:
        ...
        quote (ta_lib.objs.Quote): 期权对象，应该是 df 中多 K 线序列中的期权合约对象，否则返回序列值全为 nan。
    ...
    Example::
        ...
        from ta_lib import TqApi, TqAuth
        from ta_lib.ta import OPTION_GREEKS
    ...
    if not (quote.ins_class.endswith("OPTION") and quote.instrument_id == df["symbol"][0] and quote.underlying_symbol == df["symbol1"][0]):
        ...
        t = ta_lib.tafunc._get_t_series(df["datetime"], df["duration"], quote.expire_datetime)  # 到期时间
        if v is None:
            v = ta_lib.tafunc.get_impv(df["close1"], df["close"], quote.strike_price, r, 0.3, t, quote.option_class)
        d1 = ta_lib.tafunc._get_d1(df["close1"], quote.strike_price, r, v, t)
        new_df["delta"] = ta_lib.tafunc.get_delta(df["close1"], quote.strike_price, r, v, t, quote.option_class, d1)
        new_df["theta"] = ta_lib.tafunc.get_theta(df["close1"], quote.strike_price, r, v, t, quote.option_class, d1)
        new_df["gamma"] = ta_lib.tafunc.get_gamma(df["close1"], quote.strike_price, r, v, t, d1)
        new_df["vega"] = ta_lib.tafunc.get_vega(df["close1"], quote.strike_price, r, v, t, d1)
        new_df["rho"] = ta_lib.tafunc.get_rho(df["close1"], quote.strike_price, r, v, t, quote.option_class, d1)
    return new_df
...
def OPTION_VALUE(df, quote):
    ...
    Example::
        ...
        from ta_lib import TqApi, TqAuth
        from ta_lib.ta import OPTION_VALUE
...
def OPTION_IMPV(df, quote, r=0.025):
    ...
    Example::
        ...
        from ta_lib import TqApi, TqAuth
        from ta_lib.ta import OPTION_IMPV
    ...
    his_v = ta_lib.tafunc._get_volatility(df["close1"], df["duration"], quote.trading_time)
    his_v = 0.3 if math.isnan(his_v) else his_v
    t = ta_lib.tafunc._get_t_series(df["datetime"], df["duration"], quote.expire_datetime)  # 到期时间
    return pd.DataFrame(
        data=list(ta_lib.tafunc.get_impv(df["close1"], df["close"], quote.strike_price, r, his_v, t, quote.option_class)),
        columns=["impv"])
...
def VOLATILITY_CURVE(df: pd.DataFrame, quotes: dict, underlying: str, r=0.025):
    ...
    Example::
        ...
        from ta_lib import TqApi, TqAuth
        from ta_lib.ta import VOLATILITY_CURVE
    ...
    for symbol_title in symbol_titles:
        ...
        t = ta_lib.tafunc._get_t_series(df["datetime"], df["duration"], quote.expire_datetime)
        res_dict[quote.strike_price] = ta_lib.tafunc.get_impv(df[base_close_title],
